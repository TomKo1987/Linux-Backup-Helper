...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QGridLayout, QFormLayout, QLabel, QPushButton, QComboBox,
                             QCheckBox, QListWidget, QListWidgetItem, QScrollArea, QDialogButtonBox, QMessageBox,
                             QFileDialog, QInputDialog, QLineEdit, QWidget, QApplication, QSizePolicy)
...
class PackageInstallerOptions(QDialog):
    def __init__(self, parent=None):
        ...
        self.install_package_command = self.distro.get_pkg_install_cmd("")
        self.session = self.detect_session()
        self.top_label = QLabel("\nFirst you can select 'System Files' in Package Installer. These files will be copied using 'sudo', "
                                ...
                                "allows you to copy files such as 'pacman.conf' or 'smb.conf' to '/etc/'.\n\nUnder 'Installer Operations' you can specify how you would like to proceed. "
                                "Each action is executed one after the other. Uncheck actions to disable them.\n\n\nTips:\n\n"
                                "It is possible to copy to and from samba shares if samba is set up correctly. Source and/or destination must be saved as follows:\n\n"
    ...
    def setup_ui(self):
        ...
        distro_label = QLabel(f"Recognized Linux distribution: {self.distro_name} | Session: {self.session}")
    ...
    def installer_operations(self):
        ...
        package_installer_operation_text = Options.get_package_installer_operation_text(self.distro)
        ...
        for index, (label, operation_key) in enumerate(operations):
            ...
            if operation_key in arch_only_operations and not self.distro.supports_aur():
    ...
    def edit_system_files(self):
        ...
        content_widget = QWidget()
        grid_layout = QGridLayout(content_widget)
        system_files = Options.system_files or []  # Fallback für None
        self.system_files_widgets = []
        self.original_system_files = []
        max_text_width = 0
        ...
        try:
            ...
            for file in files:
                try:
                    ...
                    if not any(isinstance(item, dict) and str(item.get('source', '')) == source
            ...
            if added_files:
                ...
                msg = f"The following files have been successfully added:\n{file_list}" if len(
                    added_files) > 1 else f"'{added_files[0]}' was successfully added!"
